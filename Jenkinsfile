...
pipeline {
  ...
  stages {
    ...
    stage('CheckOutCode'){
      steps{
        git credentialsId: 'd9d4ed8f-45f8-4327-a5c4-1d9800f22b19', url: 'https://github.com/vijaymanjunathas5/jekins-ansible-dynimc-inv.git'
      }
    }
    ...
    stage('RunPlaybook') {
      steps {
        ...
        sh "ansible-playbook -i inventory/aws_ec2.yaml  playbooks/tomcat-setup.yaml -u ec2-user --private-key=$AWS_EC2_PRIVATE_KEY --limit tomcats --ssh-common-args='-o StrictHostKeyChecking=no'"
      }
    }
